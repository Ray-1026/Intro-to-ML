...
# inference
def inference(net1, net2, test_loader):
    model1 = PMG(net1, 512, 200).to(device)
    model1.load_state_dict(torch.load("PMG_101.pth"))
    model1.eval()

    model2 = PMG(net2, 512, 200).to(device)
    model2.load_state_dict(torch.load("PMG.pth"))
    model2.eval()
    ...
    with torch.no_grad():
        for i, images in enumerate(tqdm(test_loader)):
            ...
            output1, output2, output3, output_concat = model1(images)
            output_combine1 = output1 + output2 + output3 + output_concat

            output1, output2, output3, output_concat = model2(images)
            output_combine2 = output1 + output2 + output3 + output_concat

            output_combine = F.softmax(output_combine1, dim=-1) + F.softmax(
                output_combine2, dim=-1
            )
...
if __name__ == "__main__":
    ...
    net2 = resnet101(pretrained=True)
    ...
    inference(net1, net2, test_loader)
