...
class RelativeCoordPredictor(nn.Module):
    ...
    def forward(self, x):
        ...
        basic_anchor = label[basic_index, reduced_x_max_index, :].unsqueeze(
            1
        )  # (N, 1, 2)
        ...
        relative_coord_total = torch.cat(
            (relative_dist.unsqueeze(2), relative_angle.unsqueeze(2)), dim=-1
        )
...
class Part_Structure(nn.Module):
    ...
    def forward(self, hidden_states, part_inx, part_value, attention_map, struc_tokens):
        ...
        (
            ...
            position_weight,
            reduced_x_max_index,
        ) = self.relative_coord_predictor(attention_map)
    ...
    def forward(self, hidden_states, struc_tokens):
        ...
        for i, layer in enumerate(self.layer):
            ...
            if i > 8:
                ...
                hidden_states = self.part_structure(
                    hidden_states, part_inx, part_value, a_map, struc_tokens
                )
        ...
        _, part_inx, part_value, a_map = self.part_select(temp_weight)
        part_states = self.part_structure(
            part_states, part_inx, part_value, a_map, struc_tokens
        )
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, labels=None, step=0, global_step=10000):
        ...
        if labels is not None:
            ...
            contrast_loss = con_loss_new(
                part_tokens[:, 0], labels.view(-1), step, global_step
            )
...
def con_loss_new(
    ...
    positive_scores = torch.where(
        mask == 1.0, positive_scores - positive_scores, positive_scores
    )
    ...
    loss = (pos_cos_matrix * pos_label_matrix).sum() + (
        neg_cos_matrix * neg_label_matrix
    ).sum()
    loss /= B * B
